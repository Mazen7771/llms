...
placeholder="e.g., biology"
required
/> <div className="flex justify-end gap-2"> <Button type="button" variant="ghost" onClick={onCancel}>Cancel</Button> <Button type="submit" loading={loading}>
{isNew ? "Create Subject" : "Save Changes"} </Button> </div> </form>
);
}
...
): Promise<{ fileKey: string; fileType: string; fileSize: number } | null> => {
...
const postForm = (
  ...
  new Promise<any>((resolve, reject) => {
    ...
    xhr.addEventListener("abort", () => {
      clearStallTimer();
      reject(new Error("Upload stalled — no data sent for 60 seconds"));
    });
...
try {
  try {
    ...
    if (
      ...
      console.log(
        "[UPLOAD] Starting Blob put: pathname=%s size=%d isSmallFile=%s chunks=%d",
        tokenData.pathname,
        file.size,
        isSmallFile,
        totalChunks
    ...
    if (isSmallFile) {
      console.warn(
        "Blob upload unavailable for small file, using server path:",
        blobErr?.message
      );
    } else {
      throw new Error(
        `Upload failed: ${
          blobErr?.message || "Vercel Blob upload error"
        }`
      );
    }
  }
...
try {
  ...
  if (res.ok) {
    ...
    // Refresh the resource list immediately after creation so the
    // newly created resource appears without a manual browser refresh.
...
try {
  const res = await fetch("/api/admin/quizzes", {
    ...
    body: JSON.stringify({
      ...
      isActive: true,
      questions: generatedQuiz.questions.map(
        (q: any, idx: number) => ({
          prompt: q.prompt,
          type: q.type,
          marks: q.marks,
...
return ( <div className="min-h-screen bg-gray-50 dark:bg-gray-900"> <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"> <header className="mb-8"> <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
Content Management </h1> <p className="text-gray-600 dark:text-gray-400 mt-1">
Manage subjects, units, topics, and content </p> </header>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Slug: {subject.slug} · {subject._count.Unit} units
                  </p>
                </div>
              </div>
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-xs text-gray-500 dark:text-gray-400">
                                        {topic._count.Resource} resources ·{" "}
                                        {topic._count.Recording} recordings ·{" "}
                                        {topic._count.Quiz} QP & MS
                                      </p>
                                    </div>
                                  </div>
                                    ...
                                    {showRecordingForm === topic.id ? (
                                      ...
                                      >
                                        ...
                                        >
                                          ...
                                          <p className="text-xs text-gray-500 dark:text-gray-400">
                                            Upload your video to Mux or
                                            Cloudflare Stream first, then
                                            paste the Stream Video ID here.
                                            <a
                                      ...
                                      <div className="space-y-3">
                                        ...
                                        >
                                          ...
                                          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                                            Create IGCSE-aligned quizzes
                                            using AI (Gemini/DeepSeek).
                                            Select topic, question types,
                                            and difficulty.
                                          </p>
