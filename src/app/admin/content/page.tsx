...
try {
  ...
  if (res.ok) {
    ...
    // Re-fetch the resources so the UI gets the same processed
    // resource objects it would get after a full browser refresh.
...
return ( <div className="min-h-screen bg-gray-50 dark:bg-gray-900"> <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"> <header className="mb-8"> <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Content Management</h1> <p className="text-gray-600 dark:text-gray-400 mt-1">Manage subjects, units, topics, and content</p> </header>
                        ...
                        {unit.Topic.sort((a, b) => a.orderIndex - b.orderIndex).map((topic) => (
                          <div key={topic.id}>
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1">
                                <Link href={`/dashboard/${encodeURIComponent(subject.slug)}/${encodeURIComponent(unit.name.toLowerCase().replace(/\s+/g, '-'))}/${topic.id}`} target="_blank" title="View as student" onClick={(e) => e.stopPropagation()}>
                                ...
                                <div className="border-b border-gray-200 dark:border-gray-700">
                                  <nav className="flex gap-4" aria-label="Content tabs">
                                    ...
                                    <button
                                      ...
                                      className={`pb-2 px-1 text-sm font-medium border-b-2 transition-colors ${activeTab === "quizzes" ? "border-primary text-primary" : "border-transparent text-gray-500 hover:text-gray-300"}`}
                                    >
