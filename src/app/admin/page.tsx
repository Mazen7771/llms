...
// Resource URLs:
// - New Supabase resources store a full https://... URL in fileKey.
// - Older resources may still store a legacy file key and must continue
//   through /api/files/... .
...
export default function AdminContentPage() {
  ...
  const [newSubjectSlug, setNewSubjectSlug] =
    useState("");
  ...
  const [editSubjectSlug, setEditSubjectSlug] =
    useState("");
  ...
  const [topicResources, setTopicResources] =
    useState<Record<string, Resource[]>>({});

  const [topicRecordings, setTopicRecordings] =
    useState<Record<string, Recording[]>>({});
  ...
  const [uploadingRecording, setUploadingRecording] =
    useState<string | null>(null);

  const [showResourceForm, setShowResourceForm] =
    useState<string | null>(null);

  const [showRecordingForm, setShowRecordingForm] =
    useState<string | null>(null);
  ...
  const [showAIGenerate, setShowAIGenerate] =
    useState<string | null>(null);
  ...
  const handleDeleteSubject = async (
    ...
    if (
      !confirm(
        "Are you sure you want to delete this subject? This will delete all units, topics, and content within it."
      )
    ) {
      return;
    }
  ...
  const handleDeleteUnit = async (
    ...
    if (
      !confirm(
        "Are you sure you want to delete this unit? This will delete all topics and content within it."
      )
    ) {
      return;
    }
  ...
  const handleDeleteTopic = async (
    ...
    if (
      !confirm(
        "Are you sure you want to delete this topic? This will delete all resources, recordings, and quizzes within it."
      )
    ) {
      return;
    }
  ...
  //
  // IMPORTANT:
  // - No Vercel Blob is used here.
  // - The server only creates the temporary signed upload URL.
  // - The browser uploads the file directly to Supabase.
  // - FormData is used to match Supabase Storage's signed upload flow.
  const uploadResourceFile = async (
    ...
    // Ask our authenticated server route
    // for a temporary Supabase signed URL.
    const signResponse =
      await fetch(
        "/api/upload",
        {
    ...
    // Supabase signed uploads use PUT with multipart FormData
    // when the file is a browser File/Blob.
  ...
  const handleCreateResource =
    async (topicId: string) => {
      ...
      try {
        ...
        // Create resource record in the
        // existing Neon/Prisma database.
        console.log(
        ...
        if (res.ok) {
          ...
          // Re-fetch so the UI receives
          // the same processed resource data
          // as a normal page refresh.
