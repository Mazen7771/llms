...
function RegisterPageContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/login/student?callbackUrl=${encodeURIComponent(callbackUrl)}&registered=true`);
      router.refresh();
    } catch {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 overflow-hidden bg-gradient-to-br from-slate-950 via-[#0a2b28] to-slate-950">
      <a href="#register-form" className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-primary focus:text-white focus:rounded-lg focus:font-medium">
      ...
      <main id="register-form" className="w-full max-w-md relative z-10">
        <header className="text-center mb-10 animate-fade-in-up">
          ...
          <p className="text-white/70 text-lg">Join the Learning Management System</p>
        </header>
          ...
          {(urlError || error) && (
            <div className="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-300 animate-slide-in" role="alert" aria-live="assertive">
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm font-medium">{error || "An error occurred. Please try again."}</p>
              </div>
            </div>
          )}
          ...
          <div className="mt-8 p-5 rounded-2xl bg-white/5 border border-white/10 animate-fade-in" style={{ animationDelay: "300ms" }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-primary/20 flex items-center justify-center" aria-hidden="true">
                <ShieldIcon className="w-5 h-5 text-primary-light" />
              </div>
              <p className="font-semibold text-white">Registration Info</p>
            </div>
            <div className="space-y-3 text-sm text-white/70">
              ...
              <p className="text-primary-light font-medium">This registration is for new user accounts outside the pre-generated system.</p>
            </div>
          </div>
        </GlassCard>
